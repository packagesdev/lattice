...
void camera::init(float* mat, float f)
{
	...
	temp = (float) atan(1.0 / mat[5]);
	cullVec[0][0] = 0.0f;
	cullVec[0][1] = (float) cos(temp);
	cullVec[0][2] = (float) -sin(temp);
	cullVec[1][0] = 0.0f;
	cullVec[1][1] = -cullVec[0][1];
	cullVec[1][2] = cullVec[0][2];

	// left and right planes' vectors
	temp = (float) atan(1.0 / mat[0]);
	cullVec[2][0] = (float) cos(temp);
	cullVec[2][1] = 0.0f;
	cullVec[2][2] = (float) -sin(temp);
